...
app.post("/api/produtos", (req, res) => {
  ...
  res.status(201).json({               // 201 Created: recurso criado com sucesso
...
app.post("/api/movimentacoes", (req, res) => {
  ...
  if (!produtoId || !quantidade || !tipo) {
    return res.status(400).json({ mensagem: "Produto, quantidade e tipo são obrigatórios." }); // 400 Bad Request: dados incompletos
  }
